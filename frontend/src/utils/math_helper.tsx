...
class MathHelper {
    ...
    point_line_dist(x: number, y: number, l1x: number, l1y: number, l2x: number, l2y: number): number {
        const numerator = x * (l2y - l1y) + l1x * (y - l2y) + l2x * (l1y - y)
        const denominator = Math.sqrt(Math.pow(l2x - l1x, 2) + Math.pow(l2y - l1y, 2))
        return Math.abs(numerator / denominator)
    }

    point_line_dist_end_constrained(x: number, y: number, l1x: number, l1y: number, l2x: number, l2y: number): number {
        const t1 = (new MathHelper).point_line_t1(x, y, l1x, l1y, l2x, l2y)
        if (t1 <= 1 && t1 >= 0) {
            return (new MathHelper).point_line_dist(x, y, l1x, l1y, l2x, l2y)
        } else {
            const d1 = Math.sqrt(Math.pow(x - l1x, 2) + Math.pow(y - l1y, 2))
            const d2 = Math.sqrt(Math.pow(x - l2x, 2) + Math.pow(y - l2y, 2))
    ...
    line_point_from_t(t: number, l1x: number, l1y: number, l2x: number, l2y: number): number[] {
        const newx = (l2x - l1x) * t + l1x
        const newy = (l2y - l1y) * t + l1y
        return [newx, newy]
    }
